...
export async function seedDemoData(): Promise<{
  ...
  for (const im of mockImoveis) {
    ...
    const row = imovelToRow({ ...im, proprietarioId: proprietarioId || undefined }) as any;
  ...
  for (const p of mockPontos) {
    const row = pontoToRow({ ...p, imovelId: p.imovelId ? imoveisIds[p.imovelId] : undefined }) as any;
